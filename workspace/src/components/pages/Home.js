...
export default function Home(props) {
  ...
  return (
    <>
      ...
      <div
        className={
          props.deviceName === "aa01"
            ? "grid-container-sm"
            : "grid-container-sm type2"
        }
      >
        {smallTitles.map((element1, index) => {
          return (
            <div key={index} className="child-sm">
              {data.data.map((element, index) => {
                return <SmallCard key={index} data={element} alt={element1} />;
              })}
            </div>
          );
        })}
      ...
      <button
        className={props.deviceName === "aa01" ? "showMore" : "showMore type2"}
      >
        Show More
      </button>
      {/* </div> */}
